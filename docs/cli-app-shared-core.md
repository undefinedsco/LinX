...
Pod 交互的 collection / use-case / models / ORM / service 具体分层见 `docs/pod-interaction-layering.md`。本文档记录 CLI/App 共享内核原则；遇到 collection 与 shared use-case 边界争议时，以 Pod interaction layering spec 为准。
...
- shared model 的字段、RDF class、predicate、subject template 的唯一真相在 owning shared package 中；当前 LinX 共享 Pod 模型的 owning package 是 `@undefineds.co/models`，具体落点是 `namespaces.ts`、`vocab/*.vocab.ts`、schema 和 repository。
...
- 如果 `@undefineds.co/models` 已经存在对应 resource 或 repository，CLI / App 必须直接使用；如果缺少查询、upsert、resolve-by-uri 等能力，先在 models owning repository 补 repository/helper 和 contract tests，发布后再由壳层调用。
- 所有已进入 `@undefineds.co/models` 的结构化 Pod 数据读写，主路径必须通过 `drizzle-solid` 和 shared resource/repository。壳层不得为了方便在 CLI / App 内部直接解析 shared resource 的 Turtle；如果当前 auth 形态只有 token/fetch，必须在 session 适配层包成 `drizzle-solid` 可接受的 Inrupt-compatible session，而不是让业务查询分叉。
- `@undefineds.co/models` 只负责 shared business truth：RDF class、predicate、vocab、schema、relation 字段、repository/use-case contract。通用数据访问机制属于 `drizzle-solid`，包括 locator 到 IRI/base-relative id 的解析、row 到 subject/IRI/id 的解析、known full IRI 精确查找、date bucket subject template 展开、base path 拼接和 resource document/fragment 定位。
- `@undefineds.co/models` 不得导出 `build*ResourceIri`、`build*SubjectPath`、`buildFragmentResourceIri`、`resolvePodUri`、`whereByPodStorageId`、`findPodRowByStorageId`、`normalizePodBaseUrl`、`extractPodResourceId` 这类通用 Pod resource/path helper。需要定位 shared resource 时，业务代码必须使用强类型 row 的 `id`、`db.resolveRowIri/resolveRowId`、`db.findById/updateById/deleteById` 或 `db.findByIri/updateByIri/deleteByIri`；缺少能力时先在 ORM/repository 层补缺口。
- `id()` 虚拟列语义归 ORM：它表示 base-relative resource id，可以是 base-relative subject id（例如 `2026/05/07.ttl#approval_123`），不能在 models 或壳层被偷换成 fragment-only id，也不能把 full IRI 传给 `findById/updateById/deleteById`。调用方已持有 full IRI 时走 `findByIri/updateByIri/deleteByIri`；需要从强类型 row 获取 full IRI 时走 `db.resolveRowIri(resource, row)`；locator/mixed target 只能留在 repository 或 adapter 边界，不能泄漏成业务 API。
- ORM row 必须同时具备两层身份：`row.id` 是强类型 base-relative id，用于 UI key、collection key、selection state 和所有 `ById` 调用；full IRI 是 RDF subject，用于关系字段、participant/entityUri/maker/thread/chat 等 URI 字段，以及 `ByIri` 精确操作。App 代码不得从 `@id`、`subject`、`uri` 或 resource URL 手工解析业务 id；如果 row 没有 `id`，这是 repository/schema/ORM 问题，应该断言失败并修底层。
...
- Workspace kind 只表达执行现场形态：`folder` 或 `worktree`。`git repository` 是 workspace 关联的 Repository 元信息，不是第三种 workspace kind；repo 根目录执行也按 `worktree` 处理，只是 `folderPath === repoPath`。CLI/App/Symphony 不得再写入新的 `workspaceKind = git`。
...
- CLI/App 不得为 `approval/grant/audit` 字段定义自己的业务 predicate。shared 字段必须先在 `@undefineds.co/models` 的 namespace/vocab/schema 中定义清楚并发布，再由壳层消费。
...
这些语义属于 `@undefineds.co/models` 的 shared truth，CLI 和 App 不能各自重定义：
...
- 壳层 API 可以继续接收短 id，例如 `chatId`、`threadId` 或 CLI 参数里的 task key，但写入 `@undefineds.co/models`、shared archive 或跨端 runtime contract 前必须解析成语义 URI 字段。
...
- 内置快照应优先在 models owning repository 通过同步脚本更新并发布，而不是在多个端里各自手改 provider/model 词典
...
这条链路不允许出现第二套 credential 读取器。若 OIDC 场景、测试夹具或某个 runtime 不能直接传真实 Inrupt `Session`，修 session 适配层；若 shared model 缺少方便的聚合查询，修 `@undefineds.co/models` repository/helper 并发布；不要在 `apps/cli` 或 `apps/web` 里手写 `credentialResource` 的 Turtle parser。
...
- AI 配置共享导出只允许使用 `credentialResource`、`aiProviderResource`、`aiModelResource`，`*Table` 仅作为 models / drizzle-solid 历史兼容 alias
- 评审时发现单表回流，视为架构回退
...
- 是否已有 `@undefineds.co/models` resource/repository 可以直接用；如果没有，是否先把缺失 helper 补进 models 并覆盖 tests，然后发布给 LinX 消费

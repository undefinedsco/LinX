...
- 默认 Secretary 的持久 Agent key 是系统保留名 `__secretary__`，对应资源根形态为 `/agents/__secretary__/`；Solid `.meta` 只是描述该容器的存储文档，不是 Agent resource identity。`ai-secretary` 只保留为默认 Chat surface id；不要再读取或写入旧的 `/agents/ai-secretary/`、`/.data/agents/ai-secretary/` Agent 资源。

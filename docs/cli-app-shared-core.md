...
- `Agent` 是目录型资源：`row.id` 必须是 `agentResourceId(key)` 产生的 `{agentKey}/index.ttl#this`，full IRI 形如 `https://.../.data/agents/{agentKey}/index.ttl#this`，Agent Home 是同目录 `/.data/agents/{agentKey}/`。CLI/App 创建 Agent 时可以接收短 key，但写入 resource、调用 `findById/updateById`、初始化 Agent Home 时必须先转换成 `row.id`；不要恢复旧的 `/.data/agents/{agentKey}.ttl#this`。

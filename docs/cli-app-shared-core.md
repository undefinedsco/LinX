...
- ORM row 必须同时具备两层身份：`row.id` 是强类型 base-relative id，用于 UI key、collection key、selection state 和所有 `ById` 调用；full IRI 是 RDF subject，用于关系字段、participant/about/maker/thread/chat 等 URI 字段，以及 `ByIri` 精确操作。App 代码不得从 `@id`、`subject`、`uri` 或 resource URL 手工解析业务 id；如果 row 没有 `id`，这是 repository/schema/ORM 问题，应该断言失败并修底层。

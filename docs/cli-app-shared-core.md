...
- Symphony 的持久事实在 LinX 产品运行时必须以 Pod 为权威，但不是所有主体都应是纯 TTL。适合人读、会演化、像文档/报告/证据的资源优先 `Pod 文件 + meta`；适合机器推进状态机、调度、claim、查询、订阅的资源使用 TTL 结构化 resource。当前链条口径是：`Idea(file + meta) -> Issue(file + meta) -> Task(TTL) -> Delivery(TTL) -> Session/Run/RunStep(TTL) -> Report(file + meta) / Evidence(file + meta)`。本地持久镜像应从 Pod 文件/RDF 拉取为 JSON-LD 或 cache；`$LINX_HOME/symphony/*.json` 这类 runtime 私有 JSON 只允许作为 portable runtime、无 Pod 离线恢复或测试缓存，不得替代 `@undefineds.co/models` + `drizzle-solid` 主路径。`LINX_HOME` 默认是 `$SOLID_HOME/apps/linx`，`SOLID_HOME` 默认是 `~/.solid`。

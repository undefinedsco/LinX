...
- Product UI reads use `useLiveQuery` over the module-owned Collection. A
  component or hook must not recreate the same query with React Query or
  `db.select()`.
- Explicit retry may call `collection.fetch({ refetch: true })`; ordinary
  rendering reads hydrated state and does not trigger a second request.
- Business commands may coordinate multiple Collection writes. Direct ORM
  writes are reserved for atomic adapters that cannot be represented as one
  Collection mutation, and must project their result back into the owning
  Collection.
- Runtime protocol adapters such as ChatKit may own persistence-specific
  reads, but they are not a second UI cache. Their read ports should consume
  module Collections whenever the query model is identical.
- Do not retain compatibility `fetch*` wrappers once production callers have
  migrated. Tests call the same Collection boundary as production code.

## Module evidence

| Module | Real xpod boundary | Module-only unit responsibility |
| --- | --- | --- |
| Chat | Chat, thread, message, participant metadata, and Secretary resources | Account-scoped keys, storage routing, aggregate subscription, Secretary bootstrap, shared agent live query |
...
## Bounded working-set gate

Favorites, Inbox, Contacts, and Symphony use the shared bounded query model in
`@linx/stores`; Chat, Profile, Model Services, and raw Files LDP traversal remain
outside this migration by design.

The centralized contract is verified by:

```bash
yarn workspace @linx/stores vitest run \
  test/ordered-window.test.ts \
  test/pod-collection.test.ts \
  test/bounded-query-sync-perf.test.ts
```

The gate covers stable `id ASC` ties, Top-100 hydration, cursor paging, three-page
LRU residency, local and remote reorder/backfill, one-batch reconciliation for a
100-resource event burst, and 1,000/10,000-row source benchmarks with no physical
select requesting more than 101 rows.

The real private-Pod integration test is
`apps/web/src/lib/data/pod-collection.integration.test.ts`. On 2026-08-01 the
local xpod runtime retained 100 of 105 contacts and loaded the next logical page
successfully. The current xpod response cap requires two physical SELECT requests
to assemble the initial Top-100 logical page; this is tracked explicitly rather
than hidden behind a simulated one-request assertion.

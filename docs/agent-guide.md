...
- `/update` / 启动升级提示是 TUI self-update lifecycle；`linx update` 是顶层 package/plugin update。两者不要混用。任何 TUI 内重启、raw-mode handoff、退出文案抑制都必须走 shell lifecycle supervisor，不要在 feature 里直接 spawn/restore terminal。

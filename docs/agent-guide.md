...
- Pi/backend 或 shell/core design 更新后，先重新确认当前 executable surface；若能力已经由 Pi/backend 拥有，LinX 要修 forwarding/help/selector/lifecycle seam 或删除 stale duplicate，不用 LinX 顶层命令补一套旧 surface。
- 交互式低频配置优先放在 TUI slash surface；需要脚本化时放进 `linx config <section>`，不要提升成新的顶层快捷命令。典型例子：`/statusline` 和 `linx config status-line` 是合法面，`linx statusline` / `linx footer` 不是。
- `/update` / 启动升级提示是 TUI self-update lifecycle；`linx update` 是顶层 package/plugin update。两者不要混用。任何 TUI 内重启、raw-mode handoff、退出文案抑制都必须走 shell lifecycle supervisor，不要在 feature 里直接 spawn/restore terminal；升级后也必须回到 LinX executable surface，不能露出上游 `pi` banner/resume copy。
- LinX-owned TUI slash command（例如 `/update`、`/statusline`、`/rewind`、`/ai connect`、`/cd`）必须在 submit router 中被 shell 消费；如果它变成普通模型回复，修 command registration/router ordering/active-surface help，不要靠 prompt 解释命令。
- 缺少 LinX/Solid 登录态不能阻止 interactive TUI 启动；交互式启动要进入 TUI 并在壳内提供 `/login` / 登录选择器。只有 `--print`、脚本化命令或必须非交互访问 Pod 的路径才可以在启动前失败并提示先 `linx login`。
- 新增 CLI/TUI lifecycle patch 前必须先走 `docs/linx-shell-core-design.md` 的 shell seam：`interactive.init` 后置行为进 `linx-interactive-post-init.ts`，`interactive.run` 启动期行为进 `linx-interactive-run-router.ts`，update version check/notification 方法进 `linx-interactive-update-router.ts`，login UI selector/dialog 方法进 `linx-interactive-login-ui-router.ts`，interactive event/error 方法进 `linx-interactive-event-router.ts`，feature 错误展示进 `linx-interactive-error-display.ts`，runtime Pod session mutation 进 `linx-interactive-runtime-host.ts`，custom header mutation 进 `linx-interactive-header-host.ts`，fallback chat text append 进 `linx-interactive-chat-text-host.ts`，editor text/focus mutation 进 `linx-interactive-editor-text-host.ts`，streaming message cleanup 进 `linx-interactive-streaming-message-host.ts`，terminal-title/rendering patch 进 shell rendering seam，editor component rebinding 和临时 dialog/editor component mount/restore 进 `linx-editor-component-router.ts`，extension UI context augmentation 进 `linx-extension-ui-context-router.ts`，session work control 进 `linx-session-work-control.ts`，session thinking capability 进 `linx-session-thinking-capability-router.ts`，session metadata read 进 `linx-session-metadata.ts`，session/runtime cwd mutation 进 `linx-session-cwd-router.ts`，stop/submit/input/session patch 进各自 router；feature 模块不要直接包 Pi 方法或直接操作 Pi/TUI 容器字段。
...
- 只有 session-manager 构造/启动规划、`linx-session-metadata.ts`、`linx-session-history.ts`、runtime archive 诊断、Pod mirror / session-control 这类明确桥接 Pi archive 的模块可以直接接触 `sessionManager`；Pod projection mapping helper 也不得接收 Pi `SessionManager`，只能接收显式 archive DTO；其他 feature/rendering 模块需要新能力时先补 seam 和 boundary test。

...
- 新增 CLI/TUI lifecycle patch 前必须先走 `docs/linx-shell-core-design.md` 的 shell seam：`interactive.init` 后置行为进 `linx-interactive-post-init.ts`，`interactive.run` 启动期行为进 `linx-interactive-run-router.ts`，stop/submit/input/session patch 进各自 router；feature 模块不要直接包 Pi 方法。

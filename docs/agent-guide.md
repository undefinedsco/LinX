...
- Pi session history / branch / context rebuild 是 shell session-history seam，不是 login、rewind、auth recovery 等 feature 模块的私有逻辑。需要重试、回滚、恢复 active branch、重建 `agent.state.messages` 时，feature 模块只能调用命名 seam helper，不能直接访问 `session.sessionManager`、`getBranch`、`resetLeaf`、`buildSessionContext` 等 Pi 内部形状。

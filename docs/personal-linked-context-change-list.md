...
| `@undefineds.co/xpod` | `0.3.58` | Published and consumed by LinX source. Authenticated Cloud `obj upsert --commit` plus exact-subject `obj get` read-back works from the released package, not only the source branch. | `npm view @undefineds.co/xpod@0.3.58 version` returned `0.3.58`; xpod Release run `28466251679` passed; LinX `node_modules/.bin/xpod --version` returned `0.3.58`; live package smoke wrote and read `https://id.undefineds.co/gcloud/.data/ideas/verification/linx-consume-xpod-0.3.58-20260630T183221Z.ttl#this`. |
| LinX CLI | source `0.3.32` + CLI package dependency on `@undefineds.co/xpod@0.3.58` | The CLI release package now carries the xpod CLI used by bundled Capture/Symphony/xpod-cli skills, so a global LinX install can run the AI-facing `xpod` tool without relying on the monorepo root. The already published `@undefineds.co/linx@0.3.32` remains the last npm LinX release until the next LinX tag. | Verified locally after dependency bump: `node_modules/.bin/xpod --version` = `0.3.58`; `node --test tests/plc-agent-skill-benchmark.test.mjs`; `node --test tests/cli-release-contract.test.mjs`; `yarn benchmark:plc-agent-skills`; `yarn benchmark:plc-agent-skills:codex-e2e`; release smoke install verifies installed `@undefineds.co/xpod@0.3.58`. |
...
| PLC Agent Skill Benchmark | `yarn benchmark:plc-agent-skills`; `yarn benchmark:plc-agent-skills:codex-e2e`; `yarn verify:plc-agent-skills:codex` | Repository-local benchmark gates portable Capture/Symphony skill contracts, xpod descriptor discovery/dry-run behavior, real Codex no-login local-first capture, installed Codex skill-cache freshness, and package-version drift. Authenticated Pod writes stay as an explicit smoke, not a default benchmark, because they require real Solid auth and mutate the user's Pod. | Verified locally: static skill contract, `xpod obj schemas`, `xpod obj schemas --domain symphony`, `obj describe Idea/Task`, `obj upsert --dry-run` for Idea/Task, real Codex no-login `pending_local` outbox, Codex installed-cache freshness, fallback xpod version pinned to `package.json`, and optional authenticated 0.3.58 package smoke. |
...
| PLC-3 | published: `@undefineds.co/xpod@0.3.58` | `xpod` CLI | AI agents discover current record types from models/policy instead of prompt memory; no-login capture produces durable local pending state; authenticated Cloud writes must be readable back without depending on the sidecar SPARQL query path. | Expose `obj schemas`, `obj describe`, descriptor-backed `obj upsert --dry-run/--commit`, Cloud-compatible per-field `DELETE WHERE` + `INSERT DATA`, and exact-subject direct Turtle read fallback when SPARQL query returns 400. | `src/cli/commands/obj.ts`; `tests/cli/obj.test.ts`; root platform package pins | Published release verified: xpod Release run `28466251679` succeeded; `npm view @undefineds.co/xpod@0.3.58`; LinX dependency smoke wrote/read `https://id.undefineds.co/gcloud/.data/ideas/verification/linx-consume-xpod-0.3.58-20260630T183221Z.ttl#this`. Source verification before release: `bun run test:run tests/cli/obj.test.ts`; `bun run test:run tests/cli/rdf.test.ts`; `bun run build:ts`; `bun run publish:release:dry`. | AI-side capture and Symphony writes |
...
| PLC-6 | local package consumption verified; next LinX release pending | release/integration | The feature is complete only when released packages are consumed and Codex can perform capture through discovery-first xpod/model writes. | Publish in dependency order, upgrade LinX dependencies, build/pack/smoke-install LinX, then run Codex capture acceptance against installed packages rather than local worktree wrappers. | LinX dependency versions; package release metadata; smoke install artifacts; `scripts/benchmark-plc-agent-skills.mjs`; `tests/plc-agent-skill-benchmark.test.mjs`; `tests/cli-release-contract.test.mjs` | After xpod 0.3.58 release, LinX source and CLI tarball consume the published package and pass: `node --test tests/plc-agent-skill-benchmark.test.mjs`; `node --test tests/cli-release-contract.test.mjs`; `yarn benchmark:plc-agent-skills`; `yarn benchmark:plc-agent-skills:codex-e2e`; `yarn verify:plc-agent-skills:codex`; `yarn pack:cli:release`; `node scripts/smoke-install-cli-release.mjs`. Publishing the next LinX tag remains the external release step. | feature completion |
...
and plugin reinstall/refresh. Authenticated Pod commit/read-back is covered by
an explicit package smoke against `@undefineds.co/xpod@0.3.58`; keep it out of
default CI unless the environment provides disposable Solid credentials.
...
1. Publish the next LinX release tag after the `@undefineds.co/xpod@0.3.58`
   CLI package dependency and release-smoke gate are committed, so npm users get
   the same xpod-backed skill surface verified locally.

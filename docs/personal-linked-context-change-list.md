...
| LinX CLI | source `0.3.32` + dependency bump to `@undefineds.co/xpod@0.3.58` | Source consumes the released xpod Cloud read-back fix. The already published `@undefineds.co/linx@0.3.32` remains the last npm LinX release until the next LinX tag. | Verified locally after dependency bump: `node_modules/.bin/xpod --version` = `0.3.58`; `node --test tests/plc-agent-skill-benchmark.test.mjs`; `yarn benchmark:plc-agent-skills`; `yarn benchmark:plc-agent-skills:codex-e2e`; authenticated package smoke wrote/read the verification Idea. |
...
| PLC-6 | package consumption verified; next LinX release pending | release/integration | The feature is complete only when released packages are consumed and Codex can perform capture through discovery-first xpod/model writes. | Publish in dependency order, upgrade LinX dependencies, build/pack/smoke-install LinX, then run Codex capture acceptance against installed packages rather than local worktree wrappers. | LinX dependency versions; package release metadata; smoke install artifacts; `scripts/benchmark-plc-agent-skills.mjs`; `tests/plc-agent-skill-benchmark.test.mjs` | After xpod 0.3.58 release, LinX source consumes the published package and passes: `node --test tests/plc-agent-skill-benchmark.test.mjs`; `yarn benchmark:plc-agent-skills`; `yarn benchmark:plc-agent-skills:codex-e2e`; `yarn verify:plc-agent-skills:codex`; authenticated package smoke. Build/pack/smoke-install remain the gate for the next LinX release tag. | feature completion |
...
1. Run the next LinX release build/pack/smoke-install after committing the
   `@undefineds.co/xpod@0.3.58` dependency bump, so the published LinX package
   consumes the released xpod Cloud read-back fix.

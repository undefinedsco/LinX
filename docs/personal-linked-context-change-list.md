...
| LinX CLI | released `@undefineds.co/linx@0.3.33` + CLI package dependency on `@undefineds.co/xpod@0.3.58` | The published CLI package carries the xpod CLI used by bundled Capture/Symphony/xpod-cli skills, so a global LinX install can run the AI-facing `xpod` tool without relying on the monorepo root. | Verified: CLI Release run `28468977950` succeeded; `npm view @undefineds.co/linx@0.3.33` reports latest `0.3.33` and dependency `@undefineds.co/xpod@0.3.58`; registry install smoke returned `linx --version` = `0.3.33` and installed xpod `0.3.58`. |
...
No open release follow-up remains for this slice. New cross-repo gates should be
added to the register above instead of creating side TODOs.

...
| LinX CLI | local release candidate `0.3.33` + CLI package dependency on `@undefineds.co/xpod@0.3.58` | The CLI release package now carries the xpod CLI used by bundled Capture/Symphony/xpod-cli skills, so a global LinX install can run the AI-facing `xpod` tool without relying on the monorepo root. npm latest remains `0.3.32` until the next LinX tag is pushed and CI publishes. | Verified locally after dependency bump: `node_modules/.bin/xpod --version` = `0.3.58`; `node --test tests/plc-agent-skill-benchmark.test.mjs`; `node --test tests/cli-release-contract.test.mjs`; `yarn benchmark:plc-agent-skills`; `yarn benchmark:plc-agent-skills:codex-e2e`; release smoke install verifies installed `@undefineds.co/xpod@0.3.58`. |
...
1. Publish `linx-v0.3.33` after the `@undefineds.co/xpod@0.3.58` CLI
   package dependency and release-smoke gate are committed, so npm users get

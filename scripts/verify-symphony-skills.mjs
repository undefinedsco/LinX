...
const scenarios = [
  ...
  {
    name: 'capture asks before committing and supports policy consent',
    checks: [
      ['capture', /Consent Options/],
      ['capture', /agree/],
      ['capture', /deny/],
      ['capture', /long-term agree/],
      ['capture', /Do not commit a modeled Pod write before the user answers/],
      ['capture', /If the user denies/],
      ['capture', /If the user chooses long-term agree/],
      ['capture', /CapturePolicy/],
    ],
  },
  {
    name: 'capture requests modeling shape before inventing fields',
    checks: [
      ['capture', /Modeling Questions/],
      ['capture', /Do not invent fields for a missing user-defined type/],
      ['capture', /ask the user for the minimum modeling shape/],
      ['capture', /Grill Me/],
      ['capture', /Trellis/],
      ['capture', /product watchlist/],
      ['capture', /not as an Idea/],
    ],
  },
  {
    name: 'capture separates model proposals from instance rows',
    checks: [
      ['capture', /Modeling And Rows Are Separate/],
      ['capture', /Do not put example rows, seed rows, initialRows, or concrete product instances inside proposedFields/],
      ['capture', /ModelingProposal describes the descriptor only/],
      ['capture', /row candidate/],
      ['capture', /promote.*descriptor.*active/s],
      ['capture', /Trellis and Grill Me/],
      ['capture', /separate pending row candidates/],
    ],
  },
  {

...
# 创建 Symphony task/delivery/session 控制面记录；dry-run 不启动后端
yarn workspace @undefineds.co/linx dev symphony run "检查登录链路" --backend codex --auto --dry-run

# 安装后也可以用独立入口
linx-symphony run "检查登录链路" --backend codex --auto --dry-run
linx-symphony tasks
linx-symphony sessions
linx-symphony deliveries
linx-symphony show <taskOrDeliveryOrSessionId>
```
...
- Symphony control-plane MVP 见 [`docs/agent-collaboration-model.md`](../../docs/agent-collaboration-model.md)：`Task -> Delivery -> Session` 先落本地归档，再投影到现有 auto-mode backend
...
- `linx symphony run <objective>` 会创建本地 `Task / Delivery / Session` 记录，然后把任务投影成 runtime prompt 交给 `linx --backend ...` 的同一套 auto-mode runner。
- `--dry-run` 只写归档并打印投影 prompt，用于验证建模和投递链路，不会启动 Codex/Claude/CodeBuddy。
- 归档固定写在本地 LinX home 下的 `~/.linx/symphony/`，不新增单独的产品级 home 环境变量。
- 当前 MVP 不做 daemon、不新增 Pod schema、不改 GUI/TUI 信息架构；后续正式 Pod resource 仍以 `@undefineds.co/models` 为权威。

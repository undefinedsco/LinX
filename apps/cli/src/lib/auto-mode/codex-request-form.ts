...
type FormActionResult =
  | { kind: 'updated' }
  | { kind: 'submitted'; answers: AutoModeUserInputAnswers }
...
function defaultDraft(question: AutoModeUserInputQuestion): RequestQuestionDraft {
  return {
    selectedIndex: 0,
...
function footerHint(question: AutoModeUserInputQuestion, index: number, count: number): string {
...
export class CodexRequestForm {
  ...
  constructor(private readonly questions: AutoModeUserInputQuestion[]) {
  ...
  private applyOptionKey(
    ...
    question: AutoModeUserInputQuestion,
  ): FormActionResult {
    const draft = this.drafts[this.questionIndex]
    if (!draft) {
      return { kind: 'updated' }
  ...
  private isQuestionAnswered(index: number, question: AutoModeUserInputQuestion): boolean {
  ...
  private buildAnswers(): AutoModeUserInputAnswers {
    return this.questions.reduce<AutoModeUserInputAnswers>((record, question, index) => {
      record[question.id] = {
  ...
  private answersForQuestion(index: number, question: AutoModeUserInputQuestion): string[] {
    const draft = this.drafts[index]
    if (!draft) {
      return []
    }
    ...
    return resolveAutoModeQuestionAnswer(question, raw)
  }
}

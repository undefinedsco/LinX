...
import type { AutoModeUserInputAnswers, AutoModeUserInputQuestion } from '@linx/agent-runtime/auto-mode'
...
import {
  ...
  LINX_TUI_READY_STATUS_HINT,
  LINX_TUI_RUNNING_DRAFT_HINT,
  LINX_TUI_RUNNING_FOOTER_HINT,
  LINX_TUI_AUTO_MODE_HEADER_HINT,
  LINX_TUI_AUTO_MODE_HELP_ACTIVITY,
...
export type AutoModeDisplayPhase = 'starting' | 'ready' | 'running' | 'approval' | 'question'

interface AutoModeDisplayState {
  phase: AutoModeDisplayPhase
  detail?: string
  since: number
}

type AutoModeOverlayState =
...
export interface AutoModeDisplay {
  start(): void
  updateRecord(record: AutoModeSessionRecord): void
  updateQueue(state: AutoModeQueueState): void
  bindInputController(controller: AutoModeInputController | null): void
  setPhase(phase: AutoModeDisplayPhase, detail?: string): void
  showActivity(text: string, tone?: AutoModeUiActivityTone): void
  ...
  renderEvents(events: AutoModeNormalizedEvent[]): void
  renderRawLine(stream: AutoModeEventLogEntry['stream'], line: string): void
  promptInput(prompt: string): Promise<AutoModePromptSubmission>
...
function supportsAutoModeTui(): boolean {
  if (process.env.LINX_BACKEND_PLAIN === '1' || process.env.LINX_AUTO_MODE_PLAIN === '1') {
    return false
  }
...
function statusGlyph(phase: AutoModeDisplayPhase, elapsedMs: number): string {
...
function statusLabel(phase: AutoModeDisplayPhase): string {
  switch (phase) {
    case 'starting':
      return 'Starting'
    case 'ready':
...
function statusHint(phase: AutoModeDisplayPhase): string {
  switch (phase) {
    case 'starting':
      return 'Ctrl+C to exit'
    case 'ready':
      return LINX_TUI_READY_STATUS_HINT
    case 'running':
      return 'Ctrl+C to exit'
    case 'approval':
      return 'y/s/n/c'
...
function footerHintForPhase(phase: AutoModeDisplayPhase): { emptyHint: string; draftHint: string } {
  switch (phase) {
    case 'starting':
      return {
        emptyHint: 'Starting',
    ...
    case 'ready':
      return {
        emptyHint: LINX_TUI_READY_FOOTER_HINT,
        draftHint: LINX_TUI_READY_DRAFT_HINT,
      }
    case 'running':
      return {
        emptyHint: LINX_TUI_RUNNING_FOOTER_HINT,
        draftHint: LINX_TUI_RUNNING_DRAFT_HINT,
      }
    case 'approval':
      return {
...
export function summarizeAutoModeToolCall(name: string, args?: Record<string, unknown>): string {
  const detail = args
    ? summarizeToolField(
      args.command
      ?? args.cmd
...
export function summarizeAutoModeDebugPayload(raw: unknown): { text: string; detail?: string } {
...
function editorBorderColor(phase: AutoModeDisplayPhase): string {
  switch (phase) {
    case 'starting':
      return ANSI.dim
    case 'ready':
...
export function formatAutoModeStatusLine(
  state: AutoModeDisplayState,
  width: number,
  now = Date.now(),
): string {
  const elapsedMs = Math.max(0, now - state.since)
  const parts = [
    `${statusGlyph(state.phase, elapsedMs)} ${statusLabel(state.phase)} (${formatAutoModeElapsed(elapsedMs)})`,
...
export function formatAutoModeQueueLine(queueState: AutoModeQueueState, width: number): string {
  const parts: string[] = []
...
export function formatAutoModeFooterContext(record: AutoModeSessionRecord): string {
  return [
    shortPath(record.cwd),
    `session=${shortSessionId(resolveAutoModeSessionDisplayId(record))}`,
...
export function formatAutoModeFooterLine(input: {
  width: number
  phase: AutoModeDisplayPhase
  ...
  return renderFooterLine(
    {
      ...
      context: formatAutoModeFooterContext(input.record),
    },
    input.width,
  )
}

export function formatAutoModeActivityPanelLines(input: {
  width: number
  maxHeight: number
  entries: AutoModeUiEntry[]
  ...
  const renderToolLines = (entry: AutoModeUiEntry): string[] => {
    return [applyAnsi(clipLine(`[tool] ${entry.text}`, innerWidth), ANSI.green)]
  }

  const renderStatusLines = (entry: AutoModeUiEntry): string[] => {
  ...
  const renderDebugLines = (entry: AutoModeUiEntry): string[] => {
    if (entry.kind !== 'debug') {
      return []
    }
...
export function selectAutoModeFooterSectionCounts(input: {
  totalHeight: number
  headerCount: number
  contextCount: number
  showStatus: boolean
...
export function formatAutoModeTranscriptLine(line: string, width: number): string[] {
...
class PlainAutoModeDisplay implements AutoModeDisplay {
  ...
  private queueState: AutoModeQueueState = { steeringCount: 0, followUpCount: 0 }

  constructor(record: AutoModeSessionRecord, prompt: PromptText) {
    this.record = record
    this.prompt = prompt
  }
  ...
  async chooseQuestions(questions: AutoModeUserInputQuestion[]): Promise<AutoModeUserInputAnswers> {
  ...
  showHelp(): void {
    stdout.write(`${LINX_TUI_AUTO_MODE_HELP_TEXT}\n`)
  }
  ...
  renderEvents(events: AutoModeNormalizedEvent[]): void {
    for (const event of events) {
      ...
      if (event.type === 'tool.call') {
        stdout.write(`[tool] ${summarizeAutoModeToolCall(event.name, event.arguments)}\n`)
        continue
      }
...
class TuiAutoModeDisplay implements AutoModeDisplay {
  ...
  private readonly transcript: AutoModeUiEntry[] = []
  private readonly activityEntries: AutoModeUiEntry[] = []
  ...
  private promptResolver: ((value: AutoModePromptSubmission) => void) | null = null
  private requestFormResolver: ((answers: AutoModeUserInputAnswers) => void) | null = null
  private overlay: AutoModeOverlayState | null = null
  private overlayResolver: ((value: string) => void) | null = null
  private overlayRejecter: ((error: Error) => void) | null = null
  private inputController: AutoModeInputController | null = null
  private hideToolOutput = false
  private debugMode = false
  private lastCtrlCTime = 0
  private state: AutoModeDisplayState = {
    phase: 'starting',
    detail: 'Booting auto-mode session',
    since: Date.now(),
  }

  constructor(record: AutoModeSessionRecord, promptFallback: PromptText) {
    this.record = record
  ...
  start(): void {
    ...
    this.active = true
    this.pushActivityEntry({ kind: 'note', text: `${backendDisplayName(this.record)} ready` })
    this.pushActivityEntry({ kind: 'note', text: LINX_TUI_AUTO_MODE_READY_NOTE })
  ...
  chooseQuestions(questions: AutoModeUserInputQuestion[]): Promise<AutoModeUserInputAnswers> {
    if (!this.active) {
      const plain = new PlainAutoModeDisplay(this.record, this.promptFallback)
  ...
  showHelp(): void {
    this.showActivity(LINX_TUI_AUTO_MODE_HELP_ACTIVITY)
  }
  ...
  renderEvents(events: AutoModeNormalizedEvent[]): void {
    for (const event of events) {
      ...
      if (event.type === 'tool.call') {
        this.pushActivityEntry({
          kind: 'tool',
          text: summarizeAutoModeToolCall(event.name, event.arguments),
        })
  ...
  private pushTranscriptEntry(entry: AutoModeUiEntry): void {
    if (!entry.text.trim()) {
      return
    }
  ...
  private pushActivityEntry(entry: AutoModeUiEntry): void {
    if (!entry.text.trim()) {
      return
    }
  ...
  private buildActivityPanel(width: number, maxHeight: number): string[] {
    return formatAutoModeActivityPanelLines({
      width,
      maxHeight,
  ...
  private buildHeaderLines(width: number): string[] {
    return [
      styleStatusLine(formatAutoModeHeaderLine(this.record, width)),
      applyAnsi(clipLine(LINX_TUI_AUTO_MODE_HEADER_HINT, width), ANSI.dim),
    ]
  }
  ...
  private buildQueueLines(width: number): string[] {
    ...
    return [applyAnsi(formatAutoModeQueueLine(this.queueState, width), ANSI.dim)]
  }

  private buildFooterLine(width: number): string {
    return applyAnsi(formatAutoModeFooterLine({
      width,
  ...
  private render(): void {
    ...
    const footerSectionCounts = selectAutoModeFooterSectionCounts({
      totalHeight,
    ...
    const footerLines = [
      ...
      ...(footerSectionCounts.statusCount > 0 ? [styleStatusLine(formatAutoModeStatusLine(this.state, totalWidth))] : []),
...
export function createAutoModeDisplay(record: AutoModeSessionRecord, prompt: PromptText, options: { quiet?: boolean } = {}): AutoModeDisplay {
  ...
  return new PlainAutoModeDisplay(record, prompt)
}

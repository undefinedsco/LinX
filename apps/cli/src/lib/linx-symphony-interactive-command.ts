...
function renderSymphonySecretaryProjection(input: string): string {
  return [
    ...
    'Decide whether it is ordinary chat, a durable capture signal, a change to existing work, or delegable work.',
    ...
    'Use the xpod CLI as the direct Pod tool surface for AI-side Pod inspection and mutation.',
    'Discover current resource types and descriptors through the Pod/model tool surface before mutating modeled records. Use dry-run before commit.',
    'Use `xpod obj schemas --domain symphony --json` to inspect Symphony control-plane record types.',
    'Use `xpod obj schemas --json` for general capture/personal-memory record discovery.',
    'Use `xpod obj describe <schema-or-alias> --json` before writing a modeled record.',
    'Use `xpod obj upsert --schema <schema-or-alias> --from - --dry-run --json` before any commit, with stdin as JSONL (one JSON object per line), then commit only if the plan matches the intended resource and document/source paths.',
    ...
    'Do not hand-patch TTL or guess Pod paths for modeled product resources; also do not duplicate field definitions. Use xpod/model descriptors or inspect existing links first.',
    'If this message contains a durable signal, use the capture skill and discovered capture policy to decide the concrete record type.',
    'If no matching modeled type exists, create a local pending record, CaptureDraft, or ModelingProposal instead of forcing the signal into a LinX-default category.',
    '',
    'User message:',
    input,
  ].join('\n')

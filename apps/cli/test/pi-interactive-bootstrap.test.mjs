...
test('linx interactive /symphony switches current chat peer for following messages', async (t) => {
  ...
  assert.match(projectionMessages[0].message.content, /Symphony is on\. You are operating in the Symphony control-plane domain\./)
  assert.match(projectionMessages[0].message.content, /Discover current resource types and descriptors through the Pod\/model tool surface/)
  assert.match(projectionMessages[0].message.content, /xpod obj schemas --domain symphony --json/)
  assert.match(projectionMessages[0].message.content, /xpod obj describe <schema-or-alias> --json/)
  assert.match(projectionMessages[0].message.content, /xpod obj upsert --schema <schema-or-alias> --from - --dry-run --json/)
  assert.match(projectionMessages[0].message.content, /stdin as JSONL/)
  assert.match(projectionMessages[0].message.content, /xpod rdf query --sparql/)
  assert.doesNotMatch(projectionMessages[0].message.content, /Prefer model-backed xpod obj commands for Idea, Issue, Task/)
...
test('restored auto startup effect lives in a shell startup module', async (t) => {
  const { module, cleanup } = await loadAutoModeModule('lib/linx-restored-auto-startup.ts')
  t.after(() => cleanup())

  assert.equal(typeof module.startLinxRestoredAutoAfterInit, 'function')
})

...
test('auto-mode shell does not redefine backend-native /new command', async (t) => {
  const { module, cleanup } = await loadAutoModeModule('lib/auto-mode/shell-command.ts')
  t.after(() => cleanup())

  const root = mkdtempSync(join(tmpdir(), 'linx-auto-mode-new-command-'))
  const archiveDir = join(root, 'session')
  mkdirSync(archiveDir, { recursive: true })
  t.after(() => {
    rmSync(root, { recursive: true, force: true })

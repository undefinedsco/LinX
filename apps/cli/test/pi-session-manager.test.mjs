...
test('native Pod session source reads session and messages directly from TTL resources', async (t) => {
  ...
  const sessionUrl = `${POD_BASE}/.data/session/${sessionId}.ttl`
  ...
  resources.set(`${POD_BASE}/.data/session/`, `<${sessionUrl}> .`)
  ...
  resources.set(sessionUrl, podModule.mergeManagedBlock('', {
    subject: sessionUrl,
    triples: [

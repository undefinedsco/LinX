...
test('LinxPiPodMirror persists Pi session events into Pod tables', async (t) => {
  ...
  assert.equal(rowValues.some((row) => row.name === 'basic' && row.loadPolicy === 'file-backed'), true)
  assert.equal(rowValues.some((row) => row.name === 'capture' && row.loadPolicy === 'file-backed'), true)
  ...
  assert.equal(writes.some((write) => write.table === 'skill' && write.iri.endsWith('/agents/__secretary__/skills/basic/')), true)
  assert.equal(writes.some((write) => write.table === 'skill' && write.iri.endsWith('/agents/__secretary__/skills/capture/')), true)
  ...
  assert.deepEqual(runtimeSessionRow.metadata.runtimeSnapshot.skills.map((skill) => skill.name), ['basic', 'capture', 'symphony', 'xpod-cli'])
})

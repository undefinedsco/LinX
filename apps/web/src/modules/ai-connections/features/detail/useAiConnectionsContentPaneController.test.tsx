...
vi.mock('../../app/store', () => ({
  useAiConnectionsStore: (selector: (state: { selectedProviderId: string }) => unknown) =>
...
describe('useAiConnectionsContentPaneController connection drafts', () => {
  beforeEach(() => {
    mocks.provider = {
      ...mocks.provider,
      apiKey: 'saved-key',
  ...
  it('retains API key and base URL drafts when optimistic persistence rolls back', async () => {
    ...
    const { result, rerender } = renderHook(() => useAiConnectionsContentPaneController())

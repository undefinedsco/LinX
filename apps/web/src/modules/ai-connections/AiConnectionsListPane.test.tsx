...
vi.mock('./app/store', () => ({
  useAiConnectionsStore: (selector: (state: { selectedProviderId: string, setSelectedProviderId: typeof mockSetSelectedProviderId }) => unknown) =>
    selector({
...
import { AiConnectionsListPane } from './AiConnectionsListPane'

describe('AiConnectionsListPane', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockQueryError = null
  })
  ...
  it('renders provider list without exposing the unfinished custom-provider button', () => {
    render(<AiConnectionsListPane theme="dark" />)

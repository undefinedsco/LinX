...
vi.mock('@/lib/vendor/xpod-credential', () => ({
  Credential: mocked.credentialTable,
...
function createMockDb(agent: { provider: string; model: string }) {
  const chat = {
    id: 'chat-1',
  ...
  return {
    findByLocator: vi.fn(async (table: unknown) => table === mocked.chatTable ? chat : null),
    select: vi.fn(() => ({
      from: (table: unknown) => {
        const execute = async () => {
          ...
          if (table === mocked.credentialTable) return []
          return []
        }
